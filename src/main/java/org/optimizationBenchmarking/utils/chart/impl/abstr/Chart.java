...
public class Chart extends TitledElement implements IChart {
  ...
  /** the graphic */
  final Graphics2D m_graphic;

  /** the chart driver */
  final ChartDriver m_driver;

  /** the style set */
  final IBasicStyles m_styles;
   ...
   *          the logger
   */
  protected Chart(final Graphics2D graphic, final IBasicStyles styles,
  ...
  protected void fsmFlagsAppendName(final int flagValue,
    ...
    switch (flagValue) {
      case FLAG_HAS_LEGEND_MODE: {
        append.append("legendModeSet"); //$NON-NLS-1$
        break;
      }
      default: {
