...
public class DataElement extends TitledElement implements IDataElement {
  ...
  protected void fsmFlagsAppendName(final int flagValue,
    ...
    switch (flagValue) {
      case FLAG_HAS_COLOR: {
        append.append("colorSet"); //$NON-NLS-1$
        break;
      }
      case FLAG_HAS_STROKE: {
        append.append("strokeSet"); //$NON-NLS-1$
        break;
      }
      case FLAG_HAS_DATA: {
        append.append("dataSet"); //$NON-NLS-1$
        break;
      }
      default: {
  ...
  protected synchronized final void onClose() {
    ...
    this.process(owner, driver, styles, this.m_id, this.m_title, titleFont,
        this.m_color, stroke);
