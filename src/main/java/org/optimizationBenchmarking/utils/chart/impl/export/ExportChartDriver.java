...
/**
 * A chart driver designed to work together only with graphics which also
 * implement
 * {@link org.optimizationBenchmarking.utils.text.textOutput.ITextOutput}
 * to export chart data to text files.
 */
public final class ExportChartDriver extends ChartDriver {
   ...
   */
  private static final ITextOutput __toText(final Graphics2D graphic) {
    if (graphic instanceof ITextOutput) {
      return ((ITextOutput) graphic);
    }
    throw new IllegalArgumentException((((//
    "The chart driver '" + //$NON-NLS-1$
        TextUtils.className(ExportChartDriver.class))
        + "' requires graphics which implement '") + //$NON-NLS-1$
  ...
  protected final void renderLineChart2D(final CompiledLineChart2D chart,
    ...
    if ((xAxisTitle != null) && (yAxisTitle != null)) {
      titles = (((ExportChartDriver.COMMENT_LINE_BEGIN
          + (' ' + xAxisTitle)) + ExportChartDriver.COLUMN_SEPARATOR)
          + yAxisTitle);
    } else {
      titles = null;
    }
